...
serve(async (req) => {
  ...
  try {
    ...
    if (!locationId) {
      console.error('❌ No location ID in payload')
      return new Response(
    ...
    // Find integration by location_id
    console.log('🔍 Looking for integration with location:', locationId)
    ...
    const { data: integration, error: integrationError } = await supabase
      ...
      .eq('type', 'ghl')
      .eq('status', 'connected')
      .or(`config->>location_id.eq.${locationId}`)
      .maybeSingle()
    ...
    if (!integration) {
      console.error('❌ No integration found for location:', locationId)
      return new Response(
        JSON.stringify({ 
          error: 'Integration not found', 
          location_id: locationId,
          message: 'Please connect your GoHighLevel account first'
        }), 
    ...
    const checkinData = {
      ...
      source: 'ghl_webhook',
      source_id: formId || null,
      integration_id: integration.id,
      metadata: {
        ghl_contact_id: payload.contactId || contact.id,
