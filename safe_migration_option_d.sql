-- SAFE MIGRATION OPTION D: Column Rename Approach
...
CREATE POLICY "Team members can view coach's clients" ON public.clients
    FOR SELECT USING (
        coach_id IN (
            SELECT tm.coach_id FROM team_members tm 
            WHERE tm.member_id = auth.uid() AND tm.status = 'active'
        )
    );
...
RAISE NOTICE 'Successfully renamed client_id to old_client_id and added new client_id column';
